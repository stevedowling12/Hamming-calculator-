...
it('finds no difference between indentical strands', function () {
...
it('can compute small hamming distance', function () {
	expect(hamming.compute('AT','CT')).toEqual(1);
});

it('can compute small hamming distance in longer strands', function () {
	expect(hamming.compute('GGACG','GGTCG')).toEqual(1);
});

it('can compute large hamming distance', function () {
	expect(hamming.compute('GATACA','GCATAA')).toEqual(4);
});

it('can compute hamming distance in very long strands', function () {
	expect(hamming.compute('GGACGGATTCTG','AGGACGGATTCT')).toEqual(9);
});

it('throws an error when strands are not equal', function () {
